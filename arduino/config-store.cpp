...
void ConfigStore::clearAll() {
  Config c{}; EEPROM.put(EEPROM_ADDR, c);
}

void ConfigStore::saveWifiOnly(const char* ssid, const char* pass, const Config* prev) {
  Config c{};
  if (prev) c = *prev; // preserve any MQTT fields already stored
...
void ConfigStore::clearWifi() {
  Config c{};
  if (!load(c)) { /* nothing valid; keep zeros */ }
  c.ssid[0] = '␀';
  c.wpass[0] = '␀';
  save(c);
}
